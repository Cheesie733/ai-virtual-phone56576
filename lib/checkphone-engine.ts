...
async function buildCheckPhoneAppMessages(
  ...
  const browserSpecificInstruction = appId === "browser" 
    ? `\n<browser_app_instruction>\n极重要：浏览器历史记录(history)生成规则：\n1. 核心结合记忆：所有搜索和发帖必须100%结合该角色(${character.name})真实的生平记忆、性格弱点、当下与用户(${userIdentity?.name ?? "用户"})的具体情感进展！\n2. 结构形态（务必丰富）：\n   - 【求助提问帖】（强烈要求至少2条）：必须是角色自己以楼主身份在贴吧/知乎匿名发出的真实困扰或求助贴！标题要像真人在发问（例如：怎样才能不动声色牵手？、处于暧昧期惹她生气了怎么哄？）。正文要详细写明楼主的前因后果、细节吐槽或求助。\n   - 【主动搜索记录】：角色自己平时偷偷搜的小问句或好奇的事物。\n3. 网友评论区（必须包含！）：每个记录务必带有至少2~4条网友的精彩评论！请务必按以下打标格式输出在记录块中：\n   [评论1作者] 暴躁老姐\n   [评论1内容] 别装了，你就是死要面子活受罪，赶紧买好吃的去找她！\n   [评论2作者] 恋爱军师\n   [评论2内容] 楼主你先别急，教你一招...\n4. 楼主互动回复（真实感爆棚）：如果在评论区中角色自己(楼主)有回复网友，格式请写：\n   [评论3作者] 楼主\n   [评论3内容] 真的管用吗...我试试\n5. 彻底摒弃尴尬心理独白：情境和内心字段不要写空洞做作的文艺青年式总结，要写角色真实接地气的偷感或真实纠结。\n</browser_app_instruction>` 
    : "";
