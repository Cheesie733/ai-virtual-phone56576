...
async function buildCheckPhoneAppMessages(
  ...
  const restrictionInstruction = appId === "chat" ? `\n<npc_generation_instruction>\n极重要角色关系与社交生成指令：\n1. 核心人际网(绝对禁止编造)：单聊和群聊的所有NPC必须仅限：\n   - 该角色(${character.name})在世界书/设定中真实存在的亲友(如：金硕珍、闵玧其、郑号锡、金南俊、朴智旻、田柾国等)。\n   - 真实的家庭成员(妈妈、爸爸、姐姐、哥哥)。\n   - 严禁名单(发现即处罚)：绝对禁止生成“阿杰”、“雪儿”、“林依依”、“班长”、“死党”等任何系统默认假名！\n2. 聊天内容一致性：对话内容必须符合该角色真实的生平习惯。例如：如果是防弹少年团成员，对话应体现练习室、工作室、成员间的真实称呼。\n3. 社交多样性：必须生成 6-10 个单聊和 4-8 个群聊。其中必须包含至少一个【家庭群】(如：相亲相爱一家人)，成员必须是真实的家人。\n</npc_generation_instruction>` : "";
